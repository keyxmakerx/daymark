...
class SkyGlyphTest {
    ...
    fun `everything is a point until you lean all the way in`() {
        // THIS TEST WAS INVERTED IN SEPTEMBER 2026, and it used to read
        ...
        // list still names the kind." It revises `docs/SKY.md` §3.4, which drew glyphs at a month.
        //
        // It is inverted rather than deleted on purpose: the old assertion was the record that the
        // Sky used to sort a person's days into kinds of act at a zoom where a whole year is on
        // screen, and the new one has to be just as hard to change back by accident.
        assertTrue("a kind mark at the overview", !SkyDetail.drawsGlyphs(SkyDetail.DRIFT))
        assertTrue("a kind mark at a season", !SkyDetail.drawsGlyphs(SkyDetail.SEASON))
        assertTrue("a kind mark at a month", !SkyDetail.drawsGlyphs(SkyDetail.MONTH))
        assertTrue("leaning in to a day resolves nothing", SkyDetail.drawsGlyphs(SkyDetail.NIGHT))
