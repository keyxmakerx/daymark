 ...
 *
 * `docs/PLAN_2026-09-SKY-PEOPLE-TIMING.md` §2 is its design. Seven columns, and as with
 * [LifeEvent] the fields that are *not* here are most of the point.
 *
 * ## One table, not two
 *
 ...
 *
 * **A person or a community must never reach any code that reads mood.** §2: *"Correlations,
 * patterns and the cards they produce cannot receive a person or a community, groups included."*
 * "Meetup went well" as an *activity* is the person's own choice and stays in statistics; *Sam*
 * does not, and neither does *family*.
 *
 ...
 *   "last seen". §2 rules these out and the plan's *Not doing* list rules out a relationship model
 *   of any kind. A closeness field is the app grading somebody's relationships back at them.
 * - **No photo.** The plan: *"No photos"*, and the *Not doing* list: *"Photos of other people."*
 *   A mental-health diary that holds pictures of the people in someone's life is a different and
 *   much heavier object, and the people in the pictures never agreed to be in it.
 ...
 *
 * §2: *"a **who (or what) is this to you** line ... All free text in the person's words. No status
 * field ... The app stores, shows and shares it; it never reads it."* It is never parsed, never
 * matched against a vocabulary, never used to infer a group, and never fed to a rule. It is one
 * line the person wrote; the app's whole job with it is to keep it and show it back.
 *
 * ## [archived] hides, it does not delete
 *
 * §2: *"Archive hides one from the picker. Entries and notes are untouched."* Archiving takes a
 * name out of the *with* picker and nothing else. Every entry that already names them still names
 * them, every note is still there, and their page still opens. This matters more here than it does
 * for an activity: people leave, and a person who tidies a name out of a picker has not asked for
 * the record of the years they were in it to be rewritten.
 */
...
data class Person(
     ...
     * group says*.
     *
     * **Three states, not two, and the third one is the feature.** §2 asks for *"a default per
     * group (all off) and overrides per item"*. A plain `Boolean` cannot hold "I have not chosen
     * for this one" apart from "I chose no": a group default later turned on would either move
     * everybody who had never been touched (if `false` meant unset) or move nobody ever again (if
     * it meant chosen). `null` says *follow the group*, and it is what every new row starts as.
     *
