...
/**
 * The owner's key document over HTTP (#258): the wrapped key's insert-only versions, the read that
 * answers with the wrapped key or the key parameters, the key parameters superseded by the wrapped
 * key's presence, the owner's token on every route, the shape and size a document may have, and a
 * restart in between. docs/SYNC_PROTOCOL.md §1.2 and §2 are the contract restated here.
 */
class KeyDocumentRoutesTest {
    ...
    private suspend fun ApplicationTestBuilder.create(body: ByteArray, auth: String? = token) =
    ...
    /** The wrapped key the read serves: its version, from the header, and its bytes. */
    ...
    fun `every key-document route refuses a caller without the owner's token, and answers the owner`() = serve(tmpDir()) {
        ...
        // The create.
        for (auth in strangers) assertEquals(HttpStatusCode.Unauthorized, create(wrapped("FIRST"), auth).status)
    ...
    fun `the read serves the key parameters while no wrapped key exists, and the wrapped key once one does`() = serve(tmpDir()) {
        ...
        // With only key parameters there, the create is enrolment, and it is taken.
        val created = create(wrapped("ENROLLED"))
    ...
    fun `the key parameters are served until a wrapped key exists, and answer 410 from then on`() = serve(tmpDir()) {
        ...
        assertEquals(HttpStatusCode.Created, create(wrapped("ENROLLED")).status)
    ...
    @Test
    fun `the key parameters can be written until a wrapped key exists, and answer 410 from then on, leaving the file as it was`() {
        val dir = tmpDir()
        ...
        serve(dir) {
            ...
            assertEquals(HttpStatusCode.NoContent, putKeyparams(keyparams("SECOND")).status, "replaced while no wrapped key exists")
            assertContentEquals(keyparams("SECOND"), file.readBytes())
        ...
        assertContentEquals(keyparams("SECOND"), file.readBytes(), "never deleted and never overwritten")
    }

    // ── Insert-only versions ─────────────────────────────────────────────────────────────────────

    @Test
    fun `a second create is refused with 409 and the first document is unchanged byte for byte`() = serve(tmpDir()) {
        assertEquals(HttpStatusCode.Created, create(wrapped("FIRST")).status)
        val second = create(wrapped("SECOND"))
        assertEquals(HttpStatusCode.Conflict, second.status)
        assertEquals("""{"error":"a wrapped key already exists"}""", second.bodyAsText())
        ...
        assertEquals(HttpStatusCode.Conflict, create(wrapped("THIRD")).status)
    ...
    fun `of creates sent at once, exactly one is taken and the read serves that one`() = serve(tmpDir()) {
        ...
        assertTrue(answers.all { it.second == HttpStatusCode.Created || it.second == HttpStatusCode.Conflict })
    ...
    fun `the wrapped key and the superseded key parameters survive a restart`() {
        ...
        serve(dir) {
            ...
            assertEquals(HttpStatusCode.Conflict, create(wrapped("AGAIN")).status)
    ...
    @Test
    fun `nothing a key-document route logs carries a document, the token or the volume's path`() {
        val dir = tmpDir()
        serve(dir) {
            putKeyparams(keyparams("KDMARKSALT"))
            getDoc()
            create(wrapped("KDMARKFIRST"))
            create(wrapped("KDMARKSECOND"))
            newVersion("2", wrapped("KDMARKTHIRD"))
            newVersion("2", wrapped("KDMARKFOURTH"))
            create("[\"KDMARKFIFTH\"]".toByteArray())
        ...
        for (line in lines) {
            for (secret in listOf("KDMARK", token, dir)) assertFalse(secret in line, "a log line carries $secret: $line")
        }
    }
    ...
    fun `a volume that refuses the key documents answers 507, and logs one line per failure with no path`() {
        val dir = tmpDir()
        // A non-empty directory where the key parameters go: every read and write of them fails, even as root.
        val occupied = File(dir, KeyDocumentStore.KEYPARAMS_FILE)
        assertTrue(occupied.mkdir() && File(occupied, "occupied").createNewFile())
        serve(dir) {
            for (res in listOf(getDoc(), getKeyparams(), putKeyparams(keyparams("SALT")))) {
                assertEquals(HttpStatusCode.InsufficientStorage, res.status)
                assertEquals("""{"error":"insufficient storage"}""", res.bodyAsText())
            }
            // The positive control: the wrapped key's own file is unaffected, and it supersedes the rest.
            assertEquals(HttpStatusCode.Created, create(wrapped("ONE")).status)
            assertServesWrapped(getDoc(), 1, wrapped("ONE"))
        }
