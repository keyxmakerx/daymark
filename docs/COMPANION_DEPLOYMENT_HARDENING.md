...
| **13** | **Slowloris / missing downstream timeouts.** Netty is non-blocking so connection-count slowloris is weak, but no read/idle timeouts existed at either layer. | ✅ **closed in the app.** `requestReadTimeoutSeconds = 120` is now set on the Netty engine. Ktor's default for it is **0 — infinite** (read from the 3.0.3 source, not assumed), so before this there was no bound anywhere in the deployment once the bundled proxy's `read_body` left with it. 120 s rather than 10 because a 25 MiB snapshot over a slow mobile uplink is a legitimate long request. `responseWriteTimeoutSeconds` is left at Ktor's 10 s default — shipped behaviour with no finding against it. The `pids` limit and Netty's non-blocking model still apply on top. §3.1 requirement 9 still asks the operator's proxy for header-read timeouts, which the app cannot enforce for itself, but the deployment no longer *depends* on a proxy it cannot see. |
