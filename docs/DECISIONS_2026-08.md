...
| Not building | Why |
|---|---|
| **Streaks, points, badges, levels** | Gamification was non-significant as a moderator in the largest meta-analysis — evidence of no effect, not absence of it. Streaks turn *the log* into the goal and amplify self-blame after a break. If continuity is shown, use a non-consecutive form ("12 of the last 30 days") with no breakable state. **Done, everywhere — this row is no longer aspirational.** `MoodStats.daysWithEntryInLast30` and `stats.ts`'s `daysWithEntryLast30` are the one window (thirty days ending today, phone and web), it is shown on Stats and the web Overview and nowhere on Home, and it is omitted entirely at zero. The achievements screen, its nine-badge catalogue, its prefs-backed unlock times, `Signals.Category.Celebration`, the `streak_milestone` and `achievement_unlocked` rules and the "Streaks and milestones" settings toggle are all deleted rather than reworded. |
